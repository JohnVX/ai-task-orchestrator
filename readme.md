...
| 拖入 task | 关联 task 到 pipeline，自动继承 task 默认超时和失败继续设置。同一 task 可在同一 pipeline 中出现多次（暂不禁止）。 |
| 配置 task | 单击 pipeline 中的 task，可覆盖超时（秒数/skip/fail）和失败继续行为（y/n），不影响 task 默认值。 |
...
在右侧 pipeline 视图中单击某个 task，在超时设置后会继续询问"失败时继续? (y/n)"，留空=继承 task 默认。
