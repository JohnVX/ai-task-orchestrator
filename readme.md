...
| GET/PUT/DELETE | `/api/pipelines/{id}` | 详情（含 task 实例及超时覆盖）/ 操作（add_task, remove_task, reorder, set_task_config, set_schedule）。remove_task 使用 `task_index` 代替 `task_name`；reorder 使用 `task_indices`（索引数组）代替 `tasks`；set_task_config 使用 `task_index` 代替 `task_name`。 |
