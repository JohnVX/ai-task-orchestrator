...
| 配置 | 运行/停止命令及超时设置（`timeout_enabled`、`timeout_seconds`、`on_timeout`）存入 `task_meta/{name}.json`，跨 pipeline 共享。 |
...
| 操作 | 规则 |
|---|---|
| 创建 | 输入名称，不重名则创建。可附带 cron 表达式（5 字段标准格式）启用定时执行。创建后可修改 schedule。 |
| 拖入 task | 关联 task 到 pipeline，自动继承 task 默认超时设置。同一 task 可在同一 pipeline 中出现多次（暂不禁止）。 |
...
Task 可设置超时，支持双层配置（task 默认 + pipeline 级覆盖）。
...
在右侧 pipeline 视图中单击某个 task，可单独为其覆盖超时设置：

- 指定超时秒数（0=禁用超时，留空=继承 task 默认）。
- 指定超时行为（`fail`/`skip`，留空=继承 task 默认）。

覆盖仅影响当前 pipeline，不改变 task 的默认值。
...
| GET/PUT/DELETE | `/api/tasks/{name}` | 详情+README / 更新命令+超时配置（timeout_enabled, timeout_seconds, on_timeout）/ 删除 |
| GET | `/api/tasks/{name}/download` | 下载 task 为 tar 包 |
| GET/POST | `/api/pipelines` | 列表 / 创建 |
| GET/PUT/DELETE | `/api/pipelines/{id}` | 详情（含 task 超时覆盖）/ 操作（add_task, remove_task, reorder, set_task_config, set_schedule）/ 删除 |
