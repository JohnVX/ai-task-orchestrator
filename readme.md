...
本工具设计用于**高信任环境**（个人使用或内部团队部署于隔离网络）。安全措施聚焦于**防呆防意外**（anti-footgun），**不设认证鉴权**：

- **无认证** — 所有 API 端点在监听端口上公开可访问
- **无 HTTPS** — 生产环境应通过反向代理（nginx, caddy）提供 TLS
- **Task 命令直接执行** — `run_command` 通过 `sh -c` 直接执行，仅上传可信来源的 task
- **LLM Agent 权限** — `opencode` 使用 `--dangerously-skip-permissions`，prompt 任务继承宿主机的完整文件系统权限
- **输入校验** — tar 解包拦截路径穿越、task 名正则校验，防止注入
...
底部栏显示 GitHub 仓库链接和 MIT License 信息。
...
| 概念 | 说明 |
|---|---|
| **Task** | 用户上传的 tar 包。Task name 取自文件名，全局唯一，不可改名。支持两种类型：`self-contained`（默认，自包含可执行脚本）和 `llm-prompt`（LLM 提示词任务，包含 `prompt.md`，由 LLM Agent 执行）。类型通过 `for-task-orchestrator.txt` 的 `type:` 字段声明。`llm-prompt` task 还支持 `agent:` 字段指定独立 agent（`claude-code` 或 `opencode`），未指定则默认 `claude-code`。左侧配置面板中 `llm-prompt` task 显示 agent 下拉框，`self-contained` task 显示 run/stop 命令输入框。 |
...
- `llm_agent`: 仅 `llm-prompt` 类型 task 使用，指定执行该 task 的 LLM Agent（`claude-code` 或 `opencode`）。`self-contained` 类型为空。上传时从 `for-task-orchestrator.txt` 的 `agent:` 行读取，未指定则默认 `claude-code`。左侧配置面板可通过下拉框修改。
...
所有 JSON 文件采用原子写入（`.tmp` + rename），启动时自动清理上次崩溃遗留的 `.tmp` 文件。解析失败时仅跳过并记录日志，不阻塞服务启动。
...
| 自描述 | 包根目录下 `for-task-orchestrator.txt`，格式 `type: <type>`（可选）、`start: <cmd>` / `stop: <cmd>`（可选）、`agent: <agent-name>`（可选，仅 `llm-prompt` 类型）。上传时自动解析，`start:`/`stop:` 优先级高于默认的 `./run.sh` / `./stop.sh`。`agent:` 优先级高于全局 `--llm-agent`，未指定则默认 `claude-code`。 |
| 解析 | 在包目录下大小写不敏感查找 `README.md` / `readme.md` / `readme` / `readme.txt`，优先级：`README.md` > `readme.md` > `readme` > `readme.txt`。找到则解析展示。 |
| 配置 | `self-contained` 任务：运行/停止命令、超时设置（`timeout_enabled`、`timeout_seconds`、`on_timeout`）、失败继续（`continue_on_failure`）及超时重试（`retry_count`）。`llm-prompt` 任务：LLM Agent 下拉框（`claude-code` 或 `opencode`）、超时设置、失败继续及超时重试。存入 `task_meta/{name}.json`，跨 pipeline 共享。 |
...
| `llm-prompt` | LLM 提示词任务。包内必须包含 `prompt.md` 文件。忽略 `start:`/`stop:` 字段，由 LLM Agent 执行。可通过 `for-task-orchestrator.txt` 的 `agent:` 字段或左侧配置面板下拉框独立选择 agent（`claude-code` 或 `opencode`），未指定则默认 `claude-code`。右侧 pipeline 配置弹窗仅展示 agent（不可编辑）。stdout.log 头部记录完整 prompt 输入，后接 Agent 输出。SIGTERM 信号停止。 |

`llm-prompt` 包示例：

```
my-llm-task.tar
├── for-task-orchestrator.txt    ← 内容: type: llm-prompt\nagent: opencode  (可选，默认 claude-code)
...
| 配置 task | 单击 pipeline 中的 task 实例，弹出设置面板。`llm-prompt` 类型 task 在顶部显示当前 LLM Agent（只读，不可编辑）。可配置超时秒数、超时行为（失败/跳过）和失败继续行为（继续/停止），仅影响当前实例，不影响其他同名实例。支持一键"重置为默认"清除所有覆盖。 |
...
| 导出 | 导出 pipeline 配置为 JSON 下载（name、tasks、schedule、webhook_url、loop_count，不含运行时状态）。 |
| 导入 | 导入 JSON 文件创建新 pipeline。验证名称/tasks/cron，名称冲突返回 409。重新生成 id/created_at/status。 |
...
- `self-contained` 任务额外显示：
  - **Run Command**：执行命令（默认 `./run.sh`）。
  - **Stop Command**：停止命令（默认 `./stop.sh`）。
- `llm-prompt` 任务额外显示：
  - **LLM Agent**：下拉框选择 agent（`Claude Code` 或 `OpenCode`），默认 `claude-code`。

Task 默认超时对该 task 在所有 pipeline 中的出现均生效。

### Pipeline 级覆盖

在右侧 pipeline 视图中单击某个 task，弹出配置弹窗：

- `llm-prompt` 类型 task：顶部显示当前 LLM Agent（只读，来自左侧面板配置）。
...
| GET/PUT/DELETE | `/api/tasks/{name}` | 详情+README / 更新配置（run_command, stop_command, timeout_enabled, timeout_seconds, on_timeout, continue_on_failure, retry_count, llm_agent） / 删除 |
...
| GET | `/api/pipelines/{id}/export` | 导出 pipeline 配置为 JSON（不含运行时状态） |
| POST | `/api/pipelines/import` | 导入 pipeline 配置（multipart / JSON），验证名称/tasks/cron |
...
go.mod               — Go 1.22+
internal/
  agent/agent.go     — LLM Agent 接口、注册表、Claude Code + OpenCode 实现
  agent/agent_test.go — Agent 注册表测试（含并发安全）
  api/handler.go     — HTTP 路由与处理器
  api/handler_test.go    — HTTP API 功能测试（122 tests）
  api/contract_test.go   — 前后端 API 契约测试（17 tests）
  task/task.go       — Task 元数据（含 Type + LLMAgent 字段）、上传/下载/自描述解析（含 agent 解析）
  task/task_test.go      — Task 元数据与安全测试（44 tests）
  pipeline/pipeline.go — Pipeline CRUD 与 task 编排
  pipeline/pipeline_test.go — Pipeline CRUD 测试（6 tests）
  runner/runner.go   — 流水线执行（含 per-task Agent 查找 + 全局 fallback）、双缓冲、崩溃恢复
  runner/cron.go     — cron 表达式解析匹配
  runner/cleanup.go  — 运行数据保留策略清理
  runner/runner_test.go  — 执行器测试（34 tests）
  runner/cleanup_test.go — 清理策略测试（9 tests）
  runner/scheduler_test.go — 调度器测试（8 tests）
  logger/logger.go   — slog 日志初始化、文件轮转
  logger/logger_test.go — 日志测试（7 tests）
web/
...
```bash
go test ./... -count=1              # 全量测试 ~87s（单核）
go test ./... -count=1 -parallel=8  # 多核加速 ~23s
go test -race ./... -count=1 -parallel=8  # 竞态检测 ~24s（零 data race，零 flaky）
```

| 包 | 文件 | 测试数 | 覆盖内容 |
|---|------|--------|----------|
| `internal/api` | `handler_test.go` | 122 | HTTP API 功能测试，覆盖 task 生命周期（含 llm-prompt 上传验证、llm_agent 配置管理）、pipeline 生命周期、流水线执行（成功/超时/跳过/失败继续/手动停止/续跑/并行 stage）、run 管理、状态管理、循环执行、数据清理、cron 验证、stage 编排（set_tasks/set_task_stage/remove_task）、超时重试、并发安全 |
| `internal/api` | `contract_test.go` | 17 | 前后端 API 契约测试，验证 11 个 API 端点的响应结构（字段存在性与类型），防止后端改动导致前端 JS 运行时出错 |
| `internal/runner` | `runner_test.go` | 34 | 执行器单元测试，task 元数据读写、run 信息、日志、超时重试、循环执行、PID 复用检测、续跑迭代解析、run 目录清理 |
| `internal/runner` | `scheduler_test.go` | 8 | 调度器测试：无 schedule/无 task/运行中/定时触发/同分钟节流/cron 不匹配/删除清理/多流水线 |
| `internal/runner` | `cleanup_test.go` | 9 | 运行数据清理：禁用/限制内/超出/多流水线/运行中跳过/混合状态/空目录 |
| `internal/pipeline` | `pipeline_test.go` | 6 | Pipeline CRUD：重复 task、独立配置、索引移除、重排保留配置、边界情况、持久化、跨流水线隔离 |
| `internal/task` | `task_test.go` | 44 | Task 描述符解析（含 agent 解析、llm_agent 配置保存）、名称校验、上传验证、README 解析、tar 解包安全、元数据读写、导出、删除关联检查 |
| `internal/agent` | `agent_test.go` + `agent_race_test.go` | 8 | Agent 注册表、并发安全、并发读写竞态测试 |
| `internal/logger` | `logger_test.go` | 7 | 日志初始化（各级别+无效级别）、文件创建/追加、轮转（压缩旧文件/删除过期压缩文件/跳过近期文件） |
